  ...
  return (
    <section className="py-16 px-4 bg-[#195A22]">
      <div className="max-w-6xl mx-auto text-center mb-12">
        ...
        <h3 className="text-lg md:text-xl text-white mb-2">Find top talent or showcase your expertise at your own pace</h3>
        <p className="text-white">Work with trusted professionals or become one. It's simple, safe, and effective.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
        {steps.map((step, index) => (
          <div key={index} className="bg-gray-50 border border-gray-200 p-6 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300 text-left">
            <div className="mb-4">{step.icon}</div>
            <h2 className="text-xl font-semibold text-gray-800 mb-2">{step.title}</h2>
            <p className="text-sm text-gray-600 mb-4">{step.description}</p>
            <ReuseableButton classStyle={"bg-[#26AE61] hover:bg-[#195A22] text-white px-6 py-3 rounded-md w-full text-sm flex items-center justify-center gap-2 cursor-pointer"} text={step.buttonText} />
          </div>
        ))}
      </div>
    </section>
